...
def algorithmic_offset():
    ...
    peak_in = int(np.argmax(np.abs(sig))); peak_out = int(np.argmax(np.abs(out)))
    print(f"impulse offset: {peak_out - peak_in} samples "
          f"({(peak_out-peak_in)/den.sample_rate*1e3:.2f} ms)")
